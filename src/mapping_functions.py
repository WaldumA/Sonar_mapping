...
def imageGlobalMap(sonar_data,ekf_data,map,WALL_WIDTH,SCALE):
    # Finding Mantas current position and yaw from the EKF
    ...
    ekf_yaw = ekf_data.pose.pose.orientation.z
    #s((5000,5000),np.float32)
    ...
    for index, range in enumerate(scan_ranges):
        if range < 10:
            ...
            #Test_y,Test_x = misc.rotatePointAroundCenter(point,ekf_yaw)
            array_height,array_width = misc.rotatePointAroundCenter(point,center,ekf_yaw)
            
            print(array_height)
            print(array_width)

            #Test_x = positionManta[1] - 50*Test_x
            #Test_y = positionManta[0] - 50*Test_y

            



            #rr, cc, val = misc.getLineBetweenPoints(Test_y,Test_x,positionManta[0],positionManta[1])
            #rr, cc, val = misc.getLineBetweenPoints(array_height,array_width,positionManta[0],positionManta[1])
            #map[rr,cc] = 0.8
